            ...
            <div class="flex items-center gap-2">
              <template v-for="action in rowActions" :key="action.key">
                ...
                <Tooltip
                  ...
                  :title="isRowApiEnabled(record) ? '停用' : '启用'"
                >
                  <Button
                    type="link"
                    size="small"
                  ...
                  >
                    <IconifyIcon
                      :icon="isRowApiEnabled(record) ? 'material-symbols:toggle-on' : 'material-symbols:toggle-off'"
                      :class="isRowApiEnabled(record) ? 'text-green-500 text-2xl' : 'text-gray-400 text-2xl'"
                    />
                  </Button>
                </Tooltip>
...
/** 与通用 CRUD toggle 语义一致：is_enabled 或 RuoYi status(0=启用,1=停用) */
function isRowApiEnabled(record: any): boolean {
...
/** sys_user 主键为 user_id，其它表默认 id */
const listRowKeyField = computed(() => (props.tableCode === 'sys_user' ? 'userId' : 'id'));

function getRowPk(record: any): string | number | undefined {
  const k = listRowKeyField.value;
  const v = record?.[k];
  if (v !== undefined && v !== null && v !== '') {
    return v as string | number;
  }
  return record?.id ?? record?.userId ?? record?.user_id ?? record?.dictId ?? record?.dictCode;
}

const selectedRows = computed(() =>
  dataList.value.filter((row) => {
    const pk = getRowPk(row);
    return pk !== undefined && pk !== null && selectedRowKeys.value.includes(pk as any);
  }),
);
...
async function handleRead(record: any) {
  const id = getRowPk(record) ?? record?.dictId ?? record?.dictCode;
...
function onSelectionChange(keys: any[]) {
  ...
  const rows = dataList.value.filter((row) => {
    const pk = getRowPk(row);
    return pk !== undefined && pk !== null && keys.includes(pk as any);
  });
...
async function init() {
  try {
    ...
    const mappedFromApi = operations.map((op: any) => ({
    ...
    // 静态操作优先于接口返回（接口无数据或权限导致不可见时，页面仍可显式配置）
    metaOperations.value =
      Array.isArray(props.staticOperations) && props.staticOperations.length > 0
        ? props.staticOperations.map((op) => ({ ...op }))
        : mappedFromApi;
...
// 若外部传入静态操作按钮，覆盖 meta 配置（含首屏：与 init 中逻辑一致）
watch(
  () => props.staticOperations,
  (ops) => {
    if (ops.length) {
      metaOperations.value = ops.map((op) => ({ ...op }));
    }
  },
  { deep: true },
);
