...
type RowType = any;
/**
 * 树未返回前首屏列表兜底；进入后一律以 PT 树接口返回的一级节点 categoryCode 为准。
 */
const PROTEIN_LIST_CATEGORY_DEFAULT = 'SECT_免疫';
const PROTEIN_CATEGORY_NAME = '蛋白';
const TREE_ROOT_CATEGORY_CODE = 'PT';

function getFirstLevelCategoryCode(): string | undefined {
  const n = treeData.value?.[0] as any;
  const c = n?.categoryCode;
...
const formDrawerTitle = ref('新增蛋白产品信息');
...
const formDrawerParams = ref<Record<string, any>>({
  ...
  categoryName: PROTEIN_CATEGORY_NAME,
});
...
async function loadTree(categoryCode?: string) {
  try {
    ...
    const res = await getProductCategoryTree(code ? { category: code } : undefined);
    ...
    ajax: {
      query: async ({ page }, formValues) => {
        const code = pickListCategoryCode({
          ...
          level1Code: getFirstLevelCategoryCode(),
          fallbackCode: PROTEIN_LIST_CATEGORY_DEFAULT,
        });
...
async function openProductForm(
  ...
  const selectedCategoryCode =
    extra?.categoryCode ??
    (currentNode.value as any)?.categoryCode ??
    getFirstLevelCategoryCode() ??
    PROTEIN_LIST_CATEGORY_DEFAULT;

  if (!hasId) {
    const first = treeData.value?.[0] as any;
    const categoryName =
      (currentNode.value as any)?.categoryName ??
      (currentNode.value as any)?.categoryNameCn ??
      first?.categoryName ??
      first?.categoryNameCn ??
...
async function catalogFetchDefinition(params: Record<string, any>) {
  ...
  return getProductCatalogFormDefinition({
    ...params,
    categoryName: params?.categoryName ?? PROTEIN_CATEGORY_NAME,
  });
}
