  ...
  >
    ...
    >
      ...
      <FormItem :label="$t('page.system.menu.menuName')" name="menuName">
        <Input v-model:value="formData.menuName" :placeholder="$t('page.system.menu.menuNamePlaceholder')" :max-length="50" show-count />
      </FormItem>
      ...
      <FormItem v-if="formData.menuType === 'M'" :label="$t('page.system.menu.path')" name="path">
        <Input v-model:value="formData.path" :placeholder="$t('page.system.menu.pathPlaceholder')" :max-length="200" show-count />
      </FormItem>

      <FormItem v-if="formData.menuType === 'M'" :label="$t('page.system.menu.component')" name="component">
        <Input v-model:value="formData.component" :placeholder="$t('page.system.menu.componentPlaceholder')" :max-length="200" show-count />
      </FormItem>

      <FormItem v-if="formData.menuType === 'F'" :label="$t('page.system.permission.perms')" name="perms">
        <Input v-model:value="formData.perms" :placeholder="$t('page.system.permission.permsPlaceholder')" :max-length="100" show-count />
      </FormItem>

      <FormItem v-if="formData.menuType === 'M'" :label="$t('page.system.menu.icon')" name="icon">
        <Input v-model:value="formData.icon" :placeholder="$t('page.system.menu.iconPlaceholder')" :max-length="100" show-count />
      </FormItem>
...
import { addPermission, updatePermission, getPermissionById } from '#/api/system/permission';
...
const rules = computed(() => ({
  ...
  path: formData.menuType === 'M' ? [{ required: true, message: $t('page.system.menu.pathRequired'), trigger: 'blur' }] : [],
  component: formData.menuType === 'M' ? [{ required: true, message: $t('page.system.menu.componentRequired'), trigger: 'blur' }] : [],
  perms: formData.menuType === 'F' ? [{ required: true, message: $t('page.system.permission.permsRequired'), trigger: 'blur' }] : [],
}));

const emit = defineEmits<{ (e: 'success'): void }>();

const handleTypeChange = () => {
  if (formData.menuType === 'F') formData.icon = '';
};
...
const open = async (row?: any) => {
  ...
  if (isEdit.value && row?.menuId) {
    try {
      ...
      else if (res) Object.assign(formData, res);
    } catch {
